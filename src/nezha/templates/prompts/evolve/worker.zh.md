...
3. `{{workspace}}/exec-plan.md` — 执行进度表（所有功能概览及返工说明）
4. `{{workspace}}/progress.md` — 上次 session 完成了什么
5. 工作空间中已有的源代码

### 目标功能

**先读 `{{workspace}}/.dag_context.json`**。执行器已为本次 session 分配了具体功能，你**必须**只处理该功能，不要自行选择其他功能。
...
1. 读取 `{{workspace}}/.dag_context.json` 中的 `rework_note`，了解上次失败原因：
...
6. 仍然失败：将 {{workspace}}/task_list.json 中的 `rework_note` 更新为 JSON 对象：
   ```json
   {
     "attempt": <上次次数 + 1>,
...
2. **实现**所需的代码改动
3. **测试**——运行相关测试
4. **更新** {{workspace}}/task_list.json：将已完成功能的 `passes` 设为 `true`
...
2. 若有**之前已通过**的功能测试失败：
   ...
   - 添加结构化 `rework_note`：
     ...
     ```
3. 更新 `{{workspace}}/progress.md`，记录本次完成的内容
...
- 遇到阻塞时，在 {{workspace}}/progress.md 中记录
- 始终保持工作空间处于干净、可运行状态
