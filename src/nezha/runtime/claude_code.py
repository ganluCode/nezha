    ...
    def _tolerant_parse(data):  # noqa: ANN001
        try:
            ...
            logging.getLogger(__name__).info(
                ...
                data.get("type", data) if isinstance(data, dict) else data,
            )
            return None
...
async def run_session(
    ...
    """Run a single Claude Code session, yielding runtime-neutral events."""
    try:
...
class ClaudeCodeRuntime:
    """AgentRuntime implementation backed by claude-code-sdk."""
    ...
    async def run_session(
        ...
        """Build Claude options from context and run one SDK session."""
        if context.agent_config is None:
            raise ValueError("ClaudeCodeRuntime requires context.agent_config")
        cwd = context.cwd or context.workspace
        options = build_options(
            context.agent_config,
            cwd,
            context.security_hook,
            env=context.env,
            extra_mcp_servers=context.extra_mcp_servers,
        )
