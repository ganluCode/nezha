...
1. 仔细阅读**所有**输入文件，了解：
   - 项目需求（spec.md 或 PRD）
   - 功能列表（{{workspace}}/task_list.json）——这是你的任务跟踪器
...
4. 创建 `{{workspace}}/progress.md`，记录：
   - 已完成的初始化内容
   - {{workspace}}/task_list.json 中下一步可以开始的功能

### 规则
- 严格按规格说明执行，不要多做
- 使用 tech_stack.yaml 中指定的技术
- **不要**修改或删除 {{workspace}}/task_list.json 中的条目——只能将 `passes` 从 false 改为 true
