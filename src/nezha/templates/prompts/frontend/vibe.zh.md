...
1. `task_list.json` — 功能状态与进度
2. `progress.md` — 历史会话的执行记录
3. 目标目录中的源码（组件、页面等）
...
5. **更新状态**（如果改动影响了某个功能的状态）：
   - 修复了返工项 → 在 `task_list.json` 中设置 `passes: true`，移除 `rework` 和 `rework_note`
   - 引入了新问题 → 设置 `passes: false`，添加 `rework: true` 并注明原因
6. **提交**：`git add -A && git commit -m "vibe: <简要描述>"`
7. **更新进度**：在 `progress.md` 中记录本次会话的操作
...
- 如果指令不明确，做出最合理的判断，并在 `progress.md` 中记录假设
- 每次改动后都要运行构建/lint 检查
