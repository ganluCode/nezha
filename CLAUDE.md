...
agent-executor 是一个基于 Claude Code SDK 的 AI Agent 编排执行框架。核心能力：YAML 配置驱动多 Agent 协作、DAG 依赖调度、任务队列隔离、多 session 模式。
...
├── engine.py            LLM 引擎（claude-code-sdk 封装，异步 query()）
├── dag/
...
│   ├── knowledge.py     知识注入（load_knowledge: CLAUDE.md；load_project_context: workspace/project/）
...
**Claude Code 集成**（`nezha init` 生成）
```
CLAUDE.md                项目说明（@executor.yaml、@agents/*.yaml 导入）
.claude/settings.json    权限配置（Bash allow: nezha *、cat *、ls *、grep *）
.claude/skills/          16 个技能（status/feature-list/prd/review/rework/batch-features 等）
```
技能文件中的 `!`cmd`` 动态注入**必须是单条命令**，不能用 `||`、`for` 循环、管道链等复合 shell 语句（Claude Code 权限检查器会逐条解析拒绝）。

## 关键设计决策

**子进程隔离**（`pipeline/session.py`）
claude-code-sdk 的 anyio cancel scope 在同进程内连续调用时会污染 event loop。解决方案：每个 session 在独立 Python 子进程中运行（`subprocess.run([sys.executable, "-c", script])`），结果通过 `.session_result.json` 传递。这是 multi_round 能正常工作的关键。不要在同进程内连续调用 `sdk.query()`。
...
`load_project_context(project_dir)` 读 `workspace/project/` 下的所有文件，注入顺序先于 target 目录下的 CLAUDE.md。project 层约束始终优先于 target 层。
...
本项目自身的 Claude Code 开发工具在 `.claude/` 下：
...
**工作方式**：直接与 Claude Code 对话（vibecoding），复杂编码任务用 `@nezha-dev` 委托，改完用 `/test` 验证。不走 nezha 自身的 feature → planner → evolve-agent 流程（V3 可视化完成后再切换）。
...
| V2.3 | F1: Claude Code 集成 | **已完成** | `nezha init` 生成 CLAUDE.md + `.claude/skills/` + `settings.json`，16 个技能（EN/ZH） |
